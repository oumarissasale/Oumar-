   ...
   struct wav_header
{
   ...
   //  Opening a new file and privating it with WAV header
...
static bool wav_write(FILE *file, short data[], size_t length)
{
    ...
    return items == length;
      ...
      // Updateing the WAV file header and closes the file
  static bool wav_close(FILE *file)
{
   ...
   // Allocating memory for an empty song, set initial capacity to 16 notes
  song song_open(string filename)
{
    // Allocating memory for empty song
